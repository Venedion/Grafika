...
def count_raised_fingers(hand_landmarks, w, h):
    ...
    # Optimasi: hitung sekali dan gunakan untuk perbandingan
    thumb_diff = abs(thumb_tip.x - index_mcp.x)
    thumb_ip_diff = abs(thumb_ip.x - index_mcp.x)

    if thumb_diff > thumb_ip_diff:
        raised.append(0)
...
def get_bounding_box(hand_landmarks, w, h, padding=20):
    ...
    # Optimasi: gunakan numpy untuk perhitungan yang lebih cepat
...
def detect_hand(frame):
    ...
    # Optimasi: gunakan dict yang sama dan modify in-place
    result = {
        "gesture": "NONE",
        "fingerTip": None,
        "eraserCenter": None,
    ...
    try:
        # Konversi ke mp.Image - optimasi: hanya buat jika diperlukan
        ...
        # Update result in-place untuk efisiensi
        ...
        # ── NONE: gesture tidak dikenali ─────────────────────────────────────────
            ...
            result["gesture"] = "NONE"
            if finger_count > 0:  # Tetap hitung bounding box jika ada jari terdeteksi
                result["boundingBox"] = get_bounding_box(hand_landmarks, w, h)

        return result

    except Exception as e:
        # Optimasi: return result yang sudah ada, jangan buat baru
        result["gesture"] = "NONE"
        return result
